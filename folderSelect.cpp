...
public:
    ...
    void deleteFolder(){
        ...
        if (found == false){
            ...
            bool validFolder = false;
            while (validFolder == false) {
                string folder = getString("Please enter name of Folder to be deleted (Note deleting a folder just deletes it off the program folder list, not your system, no data is lost)");
                for (int i = 0; i < 10; i++) {
